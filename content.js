...
function createDownloadButton(trackElement = null) {
  ...
  if (trackElement) {
      // Check for existing buttons in the track element and its containers
...
function createGridViewDownloadButton(trackElement) {
  // Early return if no track element
  if (!trackElement) return;

  const trackInfo = getTrackInfo(trackElement);
  
  // Find the playable tile and action wrapper
  ...
  // Check for existing buttons in both the action wrapper and playable tile
...
function removeAllDuplicateButtons() {
  ...
  // Group buttons by their closest relevant container
  ...
  // Keep only the first button in each container
  let removedCount = 0;
