...
const ID3Writer = {
  ...
  async function getTrackData() {
    try {
      const urlMatch = window.location.href.match(/soundcloud\.com\/([^\/]+\/[^\/\?]+)/);
      if (!urlMatch) {
  ...
  function createDownloadButton() {
    ...
    if (!window.location.href.includes('soundcloud.com')) return;
    
    const actionsContainer = document.querySelector('.soundActions .sc-button-group');
    if (!actionsContainer) return;
  
    const existingButton = actionsContainer.querySelector('.sc-button-download');
  ...
  // Try to add the button immediately if the container already exists
  createDownloadButton();